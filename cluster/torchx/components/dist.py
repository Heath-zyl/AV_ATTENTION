...
def ddp(
    *script_args: str,
    ...
    if nnodes == 1:
        ...
        rdzv_endpoint = _noquote(f"$${macros.rank0_env}")
        master_addr = _noquote(f"$${macros.rank0_env}")
    
    print("master_addr: ", master_addr)
    node_rank_key = "TORCHX_NODE_RANK"
    node_rank = _noquote(f"$TORCHX_NODE_RANK")

    if env is None:
        env = {}
    env = specs.parse_env(env)
    ...
    #     "--rdzv_backend=c10d",
    #     "--rdzv_endpoint={}:{}".format(master_addr,str(rdzv_port)),
    #     f"--nnodes={nnodes}",
    ...
    return specs.AppDef(
        ...
        roles=[
            specs.Role(
                ...
                tolerations = specs.parse_tolerations(tolerations),
                node_selectors = specs.parse_node_selectors(selectors)
            )
        ],
    )
...
def _args_join(args: Iterable[str]) -> str:
    ...
    quoted = [arg if isinstance(arg, _noquote) else arg for arg in args]
    return " ".join(quoted)
