...
def role_to_pod(name: str, role: Role, service_account: Optional[str]) -> "V1Pod":
    ...
    for selector in role.node_selectors:
        node_selector[selector.key] = selector.value
    
    tolerations: list[V1Toleration] = []
    for tol in role.tolerations:
        v1t = V1Toleration(
            effect = tol.effect,
            key = tol.key,
            value = tol.value,
    ...
    address = "$VC_{}_0_HOSTS".format(cleanup_str(role.name).upper())
    cmd = "/root/wait_service.sh {}".format(address)
    init_container = V1Container(
        command=["sh", "-c", cmd],
        image="registry.aibee.cn/aibee/busybox_waitservice:1.1",
        name="wait-service",
    )

    # init_containers = []
    # if int(role.env["TORCHX_NODE_RANK"]) > 0:
...
def app_to_resource(
    ...
    for role_idx, role in enumerate(app.roles):
        for replica_id in range(role.num_replicas):
            ...
            replica_role.env["TORCHX_NODE_RANK"] = str(replica_id)
